...
func (n *Neuron) Forward(newBatch bool, batchLen int) {
	if newBatch {
		...
		n.dCdA_components = make([][]float64, batchLen)
		for i := range n.dCdA_components {
			n.dCdA_components[i] = make([]float64, len(n.Inputs))
		}
	}
	n.Output = 0
	n.Output += n.Bias
...
func (n *Neuron) Backward() {
	...
	// the previous activation. The derivative of the previous activation needs to be
...
func getActivationFunc(lType LayerType) ActivationFunc {
	switch lType {
	case Input, Output:
		return NoOp{}
	case Hidden:
